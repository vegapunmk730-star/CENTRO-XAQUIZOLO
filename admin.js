...
let editingProfissionalId = null;
let profissionalImgFile = null;

const DIAS_SEMANA = ['Domingo','Segunda','Terça','Quarta','Quinta','Sexta','Sábado'];

function renderHorariosEditor(horarios) {
  const porDia = {};
  (Array.isArray(horarios) ? horarios : []).forEach(h => {
    if (h && h.dia !== undefined) porDia[h.dia] = h;
  });

  const rows = DIAS_SEMANA.map((nome, dia) => {
    const h = porDia[dia];
    const ativo = !!h;
    return `
      <div class="hour-row" data-dia="${dia}">
        <label>
          <input type="checkbox" class="pf-dia-check" ${ativo ? 'checked' : ''}
                 onchange="this.closest('.hour-row').querySelectorAll('input[type=time]').forEach(i=>i.disabled=!this.checked)">
          ${nome}
        </label>
        <input type="time" class="pf-dia-inicio" value="${h ? esc(h.inicio || '') : '09:00'}" ${ativo ? '' : 'disabled'}>
        <span>—</span>
        <input type="time" class="pf-dia-fim" value="${h ? esc(h.fim || '') : '18:00'}" ${ativo ? '' : 'disabled'}>
      </div>`;
  }).join('');

  document.getElementById('pf_horarios_rows').innerHTML = rows;
}

function lerHorariosEditor() {
  return Array.from(document.querySelectorAll('#pf_horarios_rows .hour-row'))
    .filter(row => row.querySelector('.pf-dia-check').checked)
    .map(row => ({
      dia: parseInt(row.dataset.dia, 10),
      inicio: row.querySelector('.pf-dia-inicio').value,
      fim: row.querySelector('.pf-dia-fim').value
    }))
    .filter(h => h.inicio && h.fim);
}

async function carregarProfissionaisAdmin() {
  const area = document.getElementById('profissionaisArea');
  if (!area) return;
  ...
  try {
    ...
    if (!profissionaisData.length) {
      area.innerHTML =
        '<div class="no-data"><p>Ainda não adicionaste nenhum profissional.</p></div>';
      return;
    }
            ...
            ${profissionaisData.map(p => `
              <tr>
                <td>
                  ${
                    p.foto_url
                      ? `<img src="${esc(p.foto_url)}" style="width:44px;height:44px;object-fit:cover;border-radius:50%">`
                      : '—'
                  }
                </td>
                <td><strong style="font-weight:500">${esc(p.nome || '—')}</strong></td>
                <td style="font-size:.8rem">${esc(p.cargo || '—')}</td>
                <td style="font-size:.8rem">${esc(p.area_atuacao || '—')}</td>
                <td style="font-size:.8rem">${p.ordem ?? 0}</td>
                <td>
                  ${
                    p.ativo
                      ? '<span class="badge confirmada">Activo</span>'
                ...
                <td>
                  <button class="action-btn del" onclick="apagarProfissional('${p.id}')">Apagar</button>
                </td>
              </tr>
            `).join('')}
          </tbody>
    ...
    area.innerHTML = '<div class="no-data"><p>⚠️ Erro ao carregar profissionais.</p></div>';
  }
}

function abrirNovoProfissional() {
  editingProfissionalId = null;
  profissionalImgFile = null;

  document.getElementById('profModalTitulo').textContent = 'Novo Profissional';
  document.getElementById('pf_nome').value = '';
  document.getElementById('pf_cargo').value = '';
  document.getElementById('pf_area').value = '';
  document.getElementById('pf_descricao').value = '';
  document.getElementById('pf_biografia').value = '';
  document.getElementById('pf_foto').value = '';
  document.getElementById('pf_foto_preview').style.display = 'none';
  document.getElementById('pf_ordem').value =
    profissionaisData.length ? Math.max(...profissionaisData.map(p => p.ordem || 0)) + 1 : 0;
  document.getElementById('pf_ativo').checked = true;
  renderHorariosEditor([]);

  document.getElementById('pfErr').style.display = 'none';
  document.getElementById('pfBtn').disabled = false;
  document.getElementById('pfBtn').textContent = '💾 Guardar Profissional';

  document.getElementById('modalProfissional').classList.add('open');
}

function editarProfissional(id) {
  const p = profissionaisData.find(x => x.id === id);
  if (!p) return;

  editingProfissionalId = id;
  profissionalImgFile = null;

  document.getElementById('profModalTitulo').textContent = 'Editar Profissional';
  document.getElementById('pf_nome').value = p.nome || '';
  document.getElementById('pf_cargo').value = p.cargo || '';
  document.getElementById('pf_area').value = p.area_atuacao || '';
  document.getElementById('pf_descricao').value = p.descricao || '';
  document.getElementById('pf_biografia').value = p.biografia || '';
  document.getElementById('pf_foto').value = '';

  const prev = document.getElementById('pf_foto_preview');
  if (p.foto_url) {
    prev.src = p.foto_url;
    prev.style.display = 'block';
  } else {
  ...
  document.getElementById('pf_ordem').value = p.ordem ?? 0;
  document.getElementById('pf_ativo').checked = p.ativo !== false;
  renderHorariosEditor(p.horarios);
...
function fecharProfissional() {
  document.getElementById('modalProfissional').classList.remove('open');
}

function previewProfissionalImg() {
  const file = document.getElementById('pf_foto').files[0];
  if (!file) return;

  profissionalImgFile = file;
...
async function guardarProfissional() {
  ...
  try {
    let foto_url = editingProfissionalId
      ? (profissionaisData.find(p => p.id === editingProfissionalId) || {}).foto_url
      : null;

    if (profissionalImgFile) {
      btn.textContent = 'A enviar imagem...';

      const path = Date.now() + '-' +
        profissionalImgFile.name.replace(/[^a-zA-Z0-9.\-]/g, '_');

      const { error: upErr } = await sb
        .storage
        .from('profissionais')
        .upload(path, profissionalImgFile);
    ...
    const dados = {
      nome,
      cargo: document.getElementById('pf_cargo').value.trim() || null,
      area_atuacao: document.getElementById('pf_area').value.trim() || null,
      descricao: document.getElementById('pf_descricao').value.trim() || null,
      biografia: document.getElementById('pf_biografia').value.trim() || null,
      foto_url,
      ordem: parseInt(document.getElementById('pf_ordem').value, 10) || 0,
      ativo: document.getElementById('pf_ativo').checked,
      horarios: lerHorariosEditor(),
    ...
    btn.disabled = false;
    btn.textContent = '💾 Guardar Profissional';
  }
}

async function apagarProfissional(id) {
  if (!confirm('Apagar este profissional? Esta acção não pode ser desfeita.')) return;
